			...
			const void *value, size_t vlen)
{
	...
	case OID_id_prime256v1:
		return (ctx->curve_id == ECC_CURVE_NIST_P256 || ctx->curve_id == ECC_CURVE_SM2) ? 0 : -EINVAL;
			      ...
			      unsigned int keylen)
{
	...
	if (ret) {
		printk("asn1_ber_decoder failed in ecdsa_set_priv_key\n");
		return ret;
	}
		
	printk("asn1_ber_decoder succeed in ecdsa_set_priv_key\n");
	printk("pub key: [x] = ");
	int i = 0;
	for(i = 0; i < ctx->pub_key.ndigits; i++)
		printk("%llx", ctx->pub_key.x[i]);
	...
	printk("curve id: %d, name = %s\n", ctx->curve_id, ctx->curve->name);
	printk("curve: [x] = ");
	for(i = 0; i < ctx->curve->g.ndigits; i++)
		printk("%llx", ctx->curve->g.x[i]);
	printk(", [y] = ");
	for(i = 0; i < ctx->curve->g.ndigits; i++)
		printk("%llx", ctx->curve->g.y[i]);
	printk("\n");

	printk("[p] = ");
	for(i = 0; i < ctx->curve->g.ndigits; i++)
		printk("%llx", ctx->curve->p[i]);
	printk("\n");

	printk("[n] = ");
	for(i = 0; i < ctx->curve->g.ndigits; i++)
		printk("%llx", ctx->curve->n[i]);
	printk("\n");

	printk("[a] = ");
	for(i = 0; i < ctx->curve->g.ndigits; i++)
		printk("%llx", ctx->curve->a[i]);
	printk("\n");

	printk("[b] = ");
	for(i = 0; i < ctx->curve->g.ndigits; i++)
		printk("%llx", ctx->curve->b[i]);
	printk("\n");
	
	ecc_point_mult(&ctx->pub_key, &ctx->curve->g, ctx->d, NULL, ctx->curve,
		       ctx->curve->g.ndigits);

	//printk("now skipping the check of whether pubkey is valid ...\n");
	//ret = 0;
	ret = ecc_is_pubkey_valid_full(ctx->curve, &ctx->pub_key);
	if (ret)
		return ret;

	printk("pubkey check passed. \n");
	ctx->key_set = ret == 0;
	ctx->is_private = true;
...
static int ecdsa_set_key_noasn1_rawu64bytes(struct crypto_akcipher *tfm, const void *key,
			      unsigned int keylen)
{
	printk("ecdsa_set_key_noasn1_rawu64bytes\n"); 
	struct ecc_ctx *ctx = akcipher_tfm_ctx(tfm);
	printk("akcipher_tfm_ctx\n");

	int ret;

	ret = ecdsa_ecc_ctx_reset(ctx);

	printk("ecdsa_ecc_ctx_reset\n");
	if (ret < 0)
		return ret;

	ctx->pub_key.ndigits = 4;
	
	printk("ECC_MAX_DIGITS: %d\n", ECC_MAX_DIGITS);

	/*ctx->pub_key.x = &(ctx->x[0]);
	memcpy(ctx->pub_key.x, key + 4*8, ctx->curve->g.ndigits * sizeof(u64));
	
	ctx->pub_key.y = &(ctx->y[0]);
	memcpy(ctx->pub_key.y, key + 4*8*2, ctx->curve->g.ndigits * sizeof(u64));

	memcpy(ctx->d, key, ctx->curve->g.ndigits * sizeof(u64));*/

	printk("[Debug] use ecc_swap_digits \n");
	ctx->pub_key.x = &(ctx->x[0]);
	ecc_swap_digits((u64 *)(key + 4*8), ctx->pub_key.x, ctx->curve->g.ndigits);
	//memcpy(ctx->pub_key.x, key + 4*8, ctx->curve->g.ndigits * sizeof(u64));
	
	ctx->pub_key.y = &(ctx->y[0]);
	ecc_swap_digits((u64 *)(key + 4*8*2), ctx->pub_key.y, ctx->curve->g.ndigits);

	ecc_swap_digits((u64 *)(key), ctx->d, ctx->curve->g.ndigits);
...
static int ecdsa_set_key_noasn1(struct crypto_akcipher *tfm, const void *key,
			      unsigned int keylen)
{
	printk("ecdsa_set_key_noasn1\n");
	struct ecc_ctx *ctx = akcipher_tfm_ctx(tfm);
	printk("akcipher_tfm_ctx\n");
...
static struct akcipher_alg ecdsa_nist_p256 = {
	...
	.verify = ecdsa_verify,
	.set_priv_key = ecdsa_set_priv_key,
	.set_key_noasn1 = ecdsa_set_key_noasn1,
	.set_key_noasn1_rawu64bytes = ecdsa_set_key_noasn1_rawu64bytes,
	.set_pub_key = ecdsa_set_pub_key,
	.max_size = ecdsa_max_size,
...
static int ecdsa_sm2_init_tfm(struct crypto_akcipher *tfm)
{
	struct ecc_ctx *ctx = akcipher_tfm_ctx(tfm);

	return ecdsa_ecc_ctx_init(ctx, ECC_CURVE_SM2);
}
...
static struct akcipher_alg ecdsa_sm2 = {
	.sign = ecdsa_sign,
	.verify = ecdsa_verify,
	.set_priv_key = ecdsa_set_priv_key,
	.set_key_noasn1 = ecdsa_set_key_noasn1,
	...
	.set_pub_key = ecdsa_set_pub_key,
	.max_size = ecdsa_max_size,
	.init = ecdsa_sm2_init_tfm,
	.exit = ecdsa_exit_tfm,
	.base = {
		.cra_name = "ecdsa-sm2",
		.cra_driver_name = "ecdsa-sm2-generic",
		.cra_priority = 100,
		.cra_module = THIS_MODULE,
		.cra_ctxsize = sizeof(struct ecc_ctx),
	},
...
static int __init ecdsa_init(void)
{
	...
	printk("ecdsa_nist_p192 registered in ecdsa\n");

	ret = crypto_register_akcipher(&ecdsa_nist_p256);
	if (ret)
		goto nist_p256_error;

	printk("ecdsa_nist_p256 registered in ecdsa\n");

	ret = crypto_register_akcipher(&ecdsa_nist_p384);
	if (ret)
		goto nist_p384_error;

	printk("ecdsa_nist_p384 registered in ecdsa\n");

	ret = crypto_register_akcipher(&ecdsa_sm2);
	if (ret)
		goto sm2_error;

	printk("ecdsa_sm2 registered in ecdsa\n");

	return 0;

sm2_error:
	crypto_unregister_akcipher(&ecdsa_sm2);
